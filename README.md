...
更完整的系统重设计和交互/接口/数据模型说明见 [docs/system-redesign.md](docs/system-redesign.md)。如果你想继续扩这个项目，先看那份文档，再决定改代码，不然只会继续把语义搞散。
...
- **SSE 实时推送** — 巡检和指纹检测进度实时展示，30s 心跳保活穿透 nginx/CDN 空闲断连，断线自动重连
...
# 准备配置
cp config.toml.example config.toml       # 应用配置（proxy key 留空则启动时自动生成）
cp providers.json.example providers.json # 编辑填入你的中转站信息
...
> config.toml 与 providers.json 都含凭据，已被 .gitignore 忽略，不要提交到公开仓库。
...
[proxy]
enabled = true
api_key = "sk-relay-your-key"    # 留空则每次启动用 crypto/rand 随机生成（不持久化，重启即变；要固定就写在这里）
request_timeout = "30s"
stream_first_byte_timeout = "30s"
stream_idle_timeout = "60s"
max_retries = 2
max_request_body_bytes = 8388608      # 全局硬上限，默认 8 MiB
max_responses_body_bytes = 2097152    # /v1/responses 更严格，默认 2 MiB
```
...
| 404 | 立即换站 | 是（站点已不提供该模型，熔断后由冷却+半开探测再放行，避免每个请求都白打一次） |
| 401 / 403 | 立即换站 | 是（key 失效/额度耗尽属于持续性故障，不熔断会让它在错误率窗口每次归零后又重新吃流量，每请求白打一轮；冷却+半开探测在条件解除后再放行） |
| 413 | 立即换站 | 否（请求体过大，换站可能命中更大上下文的站点） |
| 其他 4xx | 不换站，透传给客户端 | 否 |
...
- 非流式上游响应体最多缓冲 32 MiB；超出按协议错误处理并 failover，不会让异常上游把代理内存打爆
...
- 代理现在在应用层对请求体做显式预算控制：超出 `proxy.max_request_body_bytes` 或 `proxy.max_responses_body_bytes` 时直接返回 JSON 版 `413 request_too_large`，不再让 nginx 或上游吐 HTML 错误页
...
- 全量巡检期间路由表继续使用上一份快照，巡检 **结束后统一重建一次**（而不是每完成一站重建一次）；期间站点真实故障由运行时 Circuit Breaker 兜底
...
这项目明确区分两层数据，不再把“历史记录”和“当前真相”混在一起：

- `check_results` 是 **追加式历史**，保留每次巡检/手动检查的原始结果
- `current_results` 是 **当前权威快照**，严格反映**最近一轮**检测结果，是 proxy 和模型页现在依据的唯一真相

几条保证快照“同步”的规则：

- **严格反映本轮**：全量巡检结果原样写入，不跨轮保留。某模型这轮临时失败（503/超时）就如实记为失败，不会用上一轮的旧结论冒充——也就不会出现“同一站点各模型检测时间相差好几天”的现象。
- **统一检测时间**：一个站本轮所有模型共享同一个 `checked_at`（该站检测完成的时刻），而不是逐模型各记各的。
- **死站按错误类型处理**：全量巡检时若模型列表拉取失败/为空、或全站鉴权失败（持久性错误），该站快照会被**清空**——立即从模型页首选和代理路由消失，不再拿旧模型冒充可用；连接超时这类**临时**网络错误则保留旧快照（大概率只是一次抖动）。
- **自动过期**：超过 48h 未成功刷新的快照行会被 `Cleanup`（启动时执行）清除，失败/已禁用站点的旧数据不再堆积。
- 只有**完整的全量巡检**才替换快照；`quick`、抽样、被中断的 run 只写历史，不污染当前路由。

代价：去掉跨轮保留后，8h 巡检若赶上站点瞬时抖动，该模型本轮记失败、路由短暂失去它，下一轮恢复；proxy 运行时的熔断器和实时错误率仍独立兜底实时故障。
...
[
  {
    ...
    "client_mode": "generic、codex、claude_code 或 auto",
    "codex_user_agent": "可选，仅覆盖当前站点的 Codex UA",
    "codex_originator": "可选，仅覆盖当前站点的 Codex originator",
    "claude_code_user_agent": "可选，仅覆盖当前站点的 Claude Code UA",
    "anthropic_version": "可选",
    "anthropic_beta": "可选，填 none 可禁用",
    "priority": 1.5,
    "pinned": true,
    "note": "备注信息"
  }
...
- `client_mode` 默认 `generic`；只允许 Codex CLI 的站点选 `codex`，只允许 Claude Code 的站点选 `claude_code`，同一 Key 同时提供 CC/CX 模型时选 `auto`
- `codex` 模式使用 Responses API 和 Codex CLI 身份；`claude_code` 模式使用 Anthropic Messages API。当前统一 OpenAI proxy 不做 Anthropic 协议转换，因此 Claude Code 模型只参与检测，不进入 `/v1` 路由
- CLI 身份逻辑位于独立的 `internal/clientprofile` 包。版本化 UA 和 Anthropic headers 都可以按 provider 覆盖；留空使用内置默认值。普通 `generic` provider 不会携带任何 CC/CX 特征头
- `access_token` 是 new-api 面板登录令牌，用于查询余额
- `priority` 路由倍数，默认 1.0，大于 1 则优先路由
- `name` 不能包含 `< > " ' \ `` 等特殊字符
- 同一 hostname 的重复条目在加载时合并为一个站（吸收对方缺失的 `access_token` / `last_known_quota`）；去重只发生在内存里，**加载不会改写文件**，落盘只在显式增删改时发生
...
retention_days = 7             # 历史/事件保留天数（<1 时回退默认值 7；每 24h 定时清理一次）
max_concurrency = 16           # 并发检测站点数（<1 时回退默认值 16）
request_interval = "2s"        # 站内模型间隔
ssl_verify = true              # SSL 证书验证（默认开启；仅在自签证书站点上关闭）
external_url = ""              # 外网访问地址（Proxy 页面显示用）
balance_threshold = 5.0        # 余额告警阈值

[proxy]
enabled = false                # 启用智能路由代理
api_key = ""                   # 代理 API Key（留空自动生成；勿提交真实 key 到公开仓库）
request_timeout = "30s"        # 非流式请求超时
stream_first_byte_timeout = "30s"  # 流式首字节超时（仅约束首字节，不限制整条流时长）
stream_idle_timeout = "60s"    # 流式空闲超时（流中途静默超过此值判定断流）
max_retries = 2                # 最大重试次数
max_request_body_bytes = 8388608    # 任意代理请求的硬上限（默认 8 MiB）
max_responses_body_bytes = 2097152  # /v1/responses 更严格的上限（默认 2 MiB）
```
